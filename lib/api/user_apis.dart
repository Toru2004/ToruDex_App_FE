...
class APIs {
  ...
  static Future<void> sendChatImage(AppUser chatUser, File file) async {
    ...
    try {
      final response = await cloudinary.uploadFile(
        ...
        folder:
            'ToruDex/Chats/${getConversationID(chatUser.id)}', // thư mục lưu trữ trên Cloudinary
  ...
  Future<String?> createPostOnHomePage({
    ...
    try {
      ...
      await createPostInElasticsearch(post);
  ...
  static Future<void> createPostInElasticsearch(Map<String, dynamic> post) async {
    ...
    if (response.statusCode != 200 && response.statusCode != 201) {
      print("❌ Lỗi push Elasticsearch: ${response.body}");
    } else {
      print("✅ Push Elasticsearch thành công");
    }
  }

  static Future<List<PostModel>> searchPosts(String query) async {
    final url = Uri.parse('${Config.apiUrl}/posts/search?q=$query');
